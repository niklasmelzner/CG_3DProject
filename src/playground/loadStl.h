...
public:
	IndexedObject(shared_ptr<vector<vec3>> vertices, shared_ptr<vector<vec3>> normals, shared_ptr<vector<int>> indices, shared_ptr<MTLConfig> mtlConfig) :ModelBase(indices->size(), vertices->size(), mtlConfig) {
